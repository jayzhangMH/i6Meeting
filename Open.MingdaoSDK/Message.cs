...
namespace Open.MingdaoSDK
{
    public class Message : MDAPIBase
    {
        ...
        public string SendMessage_SYS(string app_key, string app_secret, string u_id, string p_id, string msg)
        {
            if (string.IsNullOrEmpty(u_id) || string.IsNullOrEmpty(p_id) || string.IsNullOrEmpty(msg)) { return null; }
            List<Parameter> Que = new List<Parameter>();
            Que.Add(new Parameter("app_key", app_key));
            Que.Add(new Parameter("app_secret", app_secret));
            Que.Add(new Parameter("u_id", u_id));
            Que.Add(new Parameter("p_id", p_id));
            Que.Add(new Parameter("msg", msg));
            string Result = base.SyncRequest(TypeOption.MD_MESSAGE_CREATE_SYS, Que, null);
